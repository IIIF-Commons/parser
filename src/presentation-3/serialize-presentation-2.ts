...
export function languageString2to3(
  ...
  | Presentation2.LanguageProperty
  | Presentation2.LanguageProperty[]
  | undefined {
  if (!value) {
    return undefined;
...
function unNestArray<T>(
  oneOrArray: T[] | undefined,
  onlyOne = false
): T | T[] | undefined {
  if (!oneOrArray) {
    return undefined;
...
function technicalProperties(
  props: Partial<TechnicalProperties>,
  type?: string
) {
  return [
    ["@id", props.id],
    ...
    [
      "viewingDirection",
      props.viewingDirection !== "left-to-right"
        ? props.viewingDirection
        : undefined,
    ],
...
function* descriptiveProperties(
  prop: Partial<DescriptiveNormalized>
  ...
  return [
    ...
    [
      "attribution",
      prop.requiredStatement
        ? languageString2to3(prop.requiredStatement.value)
        : undefined,
    ],
  ];
}
...
export const serializeConfigPresentation2: SerializeConfig = {
  ...
  Canvas: function* (entity) {
    ...
    return [
      ...
      [
        "annotations",
        entity.annotations && entity.annotations.length
          ? unNestArray(yield entity.annotations)
          : undefined,
      ],
    ];
  },
  ...
  AnnotationPage: function* (entity) {
    return [
      ...
      [
        "resources",
        entity.items && entity.items.length
          ? unNestArray(yield entity.items)
          : undefined,
      ],
    ];
  },
  ...
  ContentResource: function* (entity: any) {
    switch (entity.type) {
      ...
      default:
        return [
          ...technicalProperties(entity, undefined),
          ...(yield* descriptiveProperties(entity)),
        ];
    }
  },
