...
describe("Has part issues", function () {
  test("merging 2 entities", () => {
    ...
    const merged = mergeEntities(initial as any, second, {
      parent: { id: "https://example.org/canvas-1" },
    });
  ...
  test("Merging 2 services", () => {
    ...
    const merged = mergeEntities(item1 as any, item2, {
      parent: { id: "https://example.org/canvas-1" },
    });
  ...
  test("Merging the same 2 entities and then different one", () => {
    ...
    const merged2 = mergeEntities(merged as any, third, {
      parent: { id: "https://example.org/canvas-3" },
    });
  ...
  describe("Collection and manifest labels", () => {
    ...
    test("Collection, then manifest.", () => {
      ...
      const merged = mergeEntities(initial as any, manifestOnItsOwn, {
        parent: undefined,
        isTopLevel: true,
      });
      expect(merged).toMatchSnapshot();
